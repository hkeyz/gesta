    ...
    <article class="sf-panel">
        ...
        <form method="POST" action="{{ route('ecommerce.cart.add', $store->slug) }}" style="display:grid; gap: 14px; margin-top: 18px;">
                    ...
                    @foreach($variations as $variation)
                        <option value="{{ $variation->id }}">
                            ...
                            @if(!empty($product->enable_stock))
                                · {{ max(0, (float) $variation->stock_qty) }} in stock
                            @endif
                        </option>
                    @endforeach
                </select>
            ...
            <div style="display:flex; gap: 12px; flex-wrap: wrap;">
                <button class="sf-button sf-button--accent" type="submit">Add to cart</button>
                <button class="sf-button" formaction="{{ route('ecommerce.buy_now.store', $store->slug) }}" type="submit">Buy now</button>
            </div>
        </form>
    </article>
</section>
